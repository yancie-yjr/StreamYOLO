...
class DFPPAFPN(nn.Module):
    ...
    def off_forward(self, input):
        ...
        # add input frame t-2
        support2_out_features = self.backbone(torch.split(input, 3, dim=1)[2])
        support2_features = [support2_out_features[f] for f in self.in_features]
        [support2_x2, support2_x1, support2_x0] = support2_features

        support2_fpn_out0 = self.lateral_conv0(support2_x0)  # 1024->512/32
        support2_f_out0 = F.interpolate(support2_fpn_out0, size=support2_x1.shape[2:4], mode='nearest')  # 512/16
        support2_f_out0 = torch.cat([support2_f_out0, support2_x1], 1)  # 512->1024/16
        support2_f_out0 = self.C3_p4(support2_f_out0)  # 1024->512/16

        support2_fpn_out1 = self.reduce_conv1(support2_f_out0)  # 512->256/16
        support2_f_out1 = F.interpolate(support2_fpn_out1, size=support2_x2.shape[2:4], mode='nearest')  # 256/8
        support2_f_out1 = torch.cat([support2_f_out1, support2_x2], 1)  # 256->512/8
        support2_pan_out2 = self.C3_p3(support2_f_out1)  # 512->256/8

        support2_p_out1 = self.bu_conv2(support2_pan_out2)  # 256->256/16
        support2_p_out1 = torch.cat([support2_p_out1, support2_fpn_out1], 1)  # 256->512/16
        support2_pan_out1 = self.C3_n3(support2_p_out1)  # 512->512/16

        support2_p_out0 = self.bu_conv1(support2_pan_out1)  # 512->512/32
        support2_p_out0 = torch.cat([support2_p_out0, support2_fpn_out0], 1)  # 512->1024/32
        support2_pan_out0 = self.C3_n4(support2_p_out0)  # 1024->1024/32

        # add input frame t-3
        support3_out_features = self.backbone(torch.split(input, 3, dim=1)[3])
        support3_features = [support3_out_features[f] for f in self.in_features]
        [support3_x2, support3_x1, support3_x0] = support3_features

        support3_fpn_out0 = self.lateral_conv0(support3_x0)  # 1024->512/32
        support3_f_out0 = F.interpolate(support3_fpn_out0, size=support3_x1.shape[2:4], mode='nearest')  # 512/16
        support3_f_out0 = torch.cat([support3_f_out0, support3_x1], 1)  # 512->1024/16
        support3_f_out0 = self.C3_p4(support3_f_out0)  # 1024->512/16

        support3_fpn_out1 = self.reduce_conv1(support3_f_out0)  # 512->256/16
        support3_f_out1 = F.interpolate(support3_fpn_out1, size=support3_x2.shape[2:4], mode='nearest')  # 256/8
        support3_f_out1 = torch.cat([support3_f_out1, support3_x2], 1)  # 256->512/8
        support3_pan_out2 = self.C3_p3(support3_f_out1)  # 512->256/8

        support3_p_out1 = self.bu_conv2(support3_pan_out2)  # 256->256/16
        support3_p_out1 = torch.cat([support3_p_out1, support3_fpn_out1], 1)  # 256->512/16
        support3_pan_out1 = self.C3_n3(support3_p_out1)  # 512->512/16

        support3_p_out0 = self.bu_conv1(support3_pan_out1)  # 512->512/32
        support3_p_out0 = torch.cat([support3_p_out0, support3_fpn_out0], 1)  # 512->1024/32
        support3_pan_out0 = self.C3_n4(support3_p_out0)  # 1024->1024/32


        # 0.5 channel
        # print("shape: ", self.jian0(rurrent_pan_out0).shape, self.jian0(support_pan_out0).shape, self.jian0(support2_pan_out0).shape)
        pan_out1 = torch.cat([self.jian1(rurrent_pan_out1), self.jian1(support_pan_out1), self.jian1(support2_pan_out1), self.jian1(support3_pan_out1)], dim=1) + rurrent_pan_out1
        pan_out2 = torch.cat([self.jian2(rurrent_pan_out2), self.jian2(support_pan_out2), self.jian2(support2_pan_out2), self.jian2(support3_pan_out2)], dim=1) + rurrent_pan_out2
        pan_out0 = torch.cat([self.jian0(rurrent_pan_out0), self.jian0(support_pan_out0), self.jian0(support2_pan_out0), self.jian0(support3_pan_out0)], dim=1) + rurrent_pan_out0
